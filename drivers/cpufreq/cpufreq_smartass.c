...
static void cpufreq_smartass_freq_change_time_work(struct work_struct *work)
{
	...
	for_each_cpu(cpu, tmp_mask) {
		...
		if (new_freq != policy->cur) {
			...
			if (cpufreq_frequency_table_target(this_smartass->cur_policy, this_smartass->freq_table,
							   new_freq, !relation,
							   &index)) {
				printk("cpufreq_frequency_table_target error: %d\n", new_freq);
				return;
			}

			new_freq = this_smartass->freq_table[index].frequency;
